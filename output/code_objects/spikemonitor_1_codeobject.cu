...
_run_kernel_spikemonitor_1_codeobject(
    ...
    int THREADS_PER_BLOCK,
        int _monitor_size,
        int32_t* _eventspace,
        int32_t* _new_ptr_array_spikemonitor_1_i,
        double* _new_ptr_array_spikemonitor_1_t,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_spikemonitor_1_N,
	int32_t* _ptr_array_neurongroup_11_i,
...
// fill the monitors
_new_ptr_array_spikemonitor_1_i[_monitor_idx] = _to_record_i;
_new_ptr_array_spikemonitor_1_t[_monitor_idx] = _to_record_t;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_spikemonitor_1_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
...
int32_t* _eventspace = dev_array_neurongroup_11__spikespace[current_idx_array_neurongroup_11__spikespace];
CUDA_SAFE_CALL(
        cudaMemcpy(
            &_num_events,
...
int _monitor_size = dev_dynamic_array_spikemonitor_1_i.size();

// Increase device vectors based on number of events
THRUST_CHECK_ERROR(
        dev_dynamic_array_spikemonitor_1_i.resize(_monitor_size + _N)
        );
THRUST_CHECK_ERROR(
        dev_dynamic_array_spikemonitor_1_t.resize(_monitor_size + _N)
        );
