#include "code_objects/statemonitor_codeobject_6.h"
#include "objects.h"
...
__global__ void
_run_kernel_statemonitor_codeobject_6(
    int _N,
    ...
    )
{
...

    {
        ...
        const double _source_I = _timedarray_18(__source_I_neurongroup_t, __source_I_neurongroup_indices);
...
void _run_statemonitor_codeobject_6()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_statemonitor_codeobject_6, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
    ...
    addresses_monitor__dynamic_array_statemonitor_v.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_v[i][0]));
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_statemonitor_codeobject_6, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_statemonitor_codeobject_6)
                );
        ...
        {
            printf("INFO _run_kernel_statemonitor_codeobject_6\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (_num__array_statemonitor__indices > 0)
{
    ...
    _run_kernel_statemonitor_codeobject_6<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
    ...
    CUDA_CHECK_ERROR("_run_kernel_statemonitor_codeobject_6");
