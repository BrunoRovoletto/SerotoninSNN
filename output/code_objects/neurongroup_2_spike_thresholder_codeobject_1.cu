#include "code_objects/neurongroup_2_spike_thresholder_codeobject_1.h"
#include "objects.h"
...
__global__ void
_run_kernel_neurongroup_2_spike_thresholder_codeobject_1(
    int _N,
    ...
    )
{
    ...
    if (_cond)
    {
        int32_t spike_index = atomicAdd(&_ptr_array_neurongroup_2__spikespace[_N], 1);
        _ptr_array_neurongroup_2__spikespace[spike_index] = _idx;
    }
    }
}

__global__ void
_reset_neurongroup_2_spike_thresholder_codeobject_1(
    int32_t* eventspace
    )
{
    using namespace brian;
...
void _run_neurongroup_2_spike_thresholder_codeobject_1()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_2_spike_thresholder_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_2_spike_thresholder_codeobject_1, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_2_spike_thresholder_codeobject_1)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_2_spike_thresholder_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
        ...
        _reset_neurongroup_2_spike_thresholder_codeobject_1<<<num_blocks, num_threads>>>(
                dev_array_neurongroup_2__spikespace[current_idx_array_neurongroup_2__spikespace]
            );

        CUDA_CHECK_ERROR("_reset_neurongroup_2_spike_thresholder_codeobject_1");

    _run_kernel_neurongroup_2_spike_thresholder_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_2__spikespace[current_idx_array_neurongroup_2__spikespace],
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_2_spike_thresholder_codeobject_1");
