#include "code_objects/synapses_45_pre_codeobject.h"
#include "objects.h"
...
int getThreadsPerBundle(){
    ...
    int threads_per_bundle = static_cast<int>(synapses_45_pre_bundle_size_max);
...
__global__ void
_run_kernel_synapses_45_pre_codeobject(
    int _N,
    int bid_offset,
    int timestep,
    ...
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_synapses_45_N,
	double* _ptr_array_neurongroup_34_Y,
	int32_t* _ptr_array_synapses_45__synaptic_post,
	const int _num_postsynaptic_idx,
	int32_t* _ptr_array_synapses_45__synaptic_pre,
    ...
    )
{
    ...
    {  // _idx is defined in outer and inner scope (for `scalar_code`)
        if (synapses_45_pre.no_or_const_delay_mode)
        {
            // TODO: pass as kernel parameter instead?
            int num_parallel_blocks = synapses_45_pre.queue->num_blocks;
            int32_t spikes_start = synapses_45_pre.spikes_start;
            int32_t spikes_stop = synapses_45_pre.spikes_stop;

            // for the first delay timesteps the eventspace is not yet filled
            if (timestep >= synapses_45_pre.queue->num_delays)
            {
                ...
                int post_block_idx = bid % num_parallel_blocks;
                {
                    ...
                    if(spikes_start <= spiking_neuron && spiking_neuron < spikes_stop)
                    {
                        ...
                        int num_synapses = synapses_45_pre_num_synapses_by_pre[pre_post_block_id];
                        int32_t* propagating_synapses = synapses_45_pre_synapse_ids_by_pre[pre_post_block_id];
                        for(int j = tid; j < num_synapses; j+=_threads_per_block)
                        {
                            ...
                            const int32_t _postsynaptic_idx = _ptr_array_synapses_45__synaptic_post[_idx];
                            double Y_post;
                            Y_post = volt;
                            _ptr_array_neurongroup_34_Y[_postsynaptic_idx] = Y_post;
        ...
        else  // heterogeneous delay mode
        {
            ...
            synapses_45_pre.queue->peek(&synapses_queue);
            ...
            for (int i = tid; i < queue_size*threads_per_bundle; i+=_threads_per_block)
            {
                ...
                int bundle_size = synapses_45_pre_num_synapses_by_bundle[bundle_id];
                int synapses_offset = synapses_45_pre_synapses_offset_by_bundle[bundle_id];
                int32_t* synapse_ids = synapses_45_pre_synapse_ids;
...
void _run_synapses_45_pre_codeobject()
{
    using namespace brian;
    ...
    const int _numN = 1;
		const int _numY_post = 16;
		int32_t* const dev_array_synapses_45__synaptic_post = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_45__synaptic_post[0]);
		const int _num_postsynaptic_idx = dev_dynamic_array_synapses_45__synaptic_post.size();
		int32_t* const dev_array_synapses_45__synaptic_pre = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_45__synaptic_pre[0]);
		const int _num_synaptic_pre = dev_dynamic_array_synapses_45__synaptic_pre.size();
		const double volt = 1.0;
...
//num_threads_per_bundle = synapses_45_pre_bundle_size_max;
num_threads_per_bundle = getThreadsPerBundle();
printf("INFO _run_kernel_synapses_45_pre_codeobject: Using %d threads per bundle\n", num_threads_per_bundle);
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_45_pre_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_45_pre_codeobject)
                );
...
else if (synapses_45_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_45_pre pathway. Skipping synapses_push and synapses kernels.\n");
}

        else
        {
            printf("INFO _run_kernel_synapses_45_pre_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_45_pre_max_size > 0)
{
    ...
    if (synapses_45_pre_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_45_pre_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_neurongroup_24__spikespace[synapses_45_pre_eventspace_idx][_num__array_neurongroup_24__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            _run_kernel_synapses_45_pre_codeobject<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads_per_bundle,
                dev_array_neurongroup_24__spikespace[synapses_45_pre_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
                dev_array_synapses_45_N,
			dev_array_neurongroup_34_Y,
			dev_array_synapses_45__synaptic_post,
			_num_postsynaptic_idx,
			dev_array_synapses_45__synaptic_pre,
			_num_synaptic_pre
            );
        }
    }

    CUDA_CHECK_ERROR("_run_kernel_synapses_45_pre_codeobject");
}


}

void _debugmsg_synapses_45_pre_codeobject()
{
    using namespace brian;
    std::cout << "Number of synapses: " << _array_synapses_45_N[0] << endl;
}
