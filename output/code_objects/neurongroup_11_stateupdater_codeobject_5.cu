#include "code_objects/neurongroup_11_stateupdater_codeobject_5.h"
#include "objects.h"
...
namespace {
    ...
    float _brian_exp(float value)
    {
        return exp(value);
                        ...
                        #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ > 0))
                        __device__ double* _namespace_timedarray_17_values;
                        #else
                        double* _namespace_timedarray_17_values;
                        #endif
    __host__ __device__
    static inline double _timedarray_17(const double t, const int i)
    {
        ...
        if(timestep < 0)
           timestep = 0;
            ...
            timestep = 40-1;
        return _namespace_timedarray_17_values[timestep*120 + i];
    }
...
__global__ void
_run_kernel_neurongroup_11_stateupdater_codeobject_5(
    int _N,
    ...
    )
{
    ...
    ///// kernel_lines /////
                        ...
                        #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ > 0))
                        _namespace_timedarray_17_values = d_timedarray_17_values;
                        #else
                        _namespace_timedarray_17_values = _timedarray_17_values;
                        #endif
...

    {
        ...
        const double _v = v + (_lio_6 * ((((((- A) + (_lio_7 * _brian_exp(_lio_8 * (_lio_9 + v)))) + Iexc) + Iexc2) + _timedarray_17(t, indices)) - (gL * (_lio_2 + v))));
        A = _A;
        X = _X;
        Y = _Y;
        v = _v;
...
void _run_neurongroup_11_stateupdater_codeobject_5()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_11_stateupdater_codeobject_5, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_11_stateupdater_codeobject_5, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_11_stateupdater_codeobject_5)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_11_stateupdater_codeobject_5\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_11_stateupdater_codeobject_5<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_11_stateupdater_codeobject_5");
