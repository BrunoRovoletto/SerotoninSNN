#include "code_objects/synapses_pre_codeobject_3.h"
#include "objects.h"
...
__global__ void
_run_kernel_synapses_pre_codeobject_3(
    int _N,
    int bid_offset,
    int timestep,
...
void _run_synapses_pre_codeobject_3()
{
    using namespace brian;
...
//num_threads_per_bundle = synapses_pre_bundle_size_max;
num_threads_per_bundle = getThreadsPerBundle();
printf("INFO _run_kernel_synapses_pre_codeobject_3: Using %d threads per bundle\n", num_threads_per_bundle);
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_pre_codeobject_3, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_pre_codeobject_3)
                );
...
else if (synapses_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO _run_kernel_synapses_pre_codeobject_3\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_pre_max_size > 0)
{
    ...
    if (synapses_pre_scalar_delay)
    {
        if (defaultclock.timestep[0] >= synapses_pre_delay)
        {
            cudaMemcpy(&num_spiking_neurons,
                    &dev_array_neurongroup_11__spikespace[synapses_pre_eventspace_idx][_num__array_neurongroup_11__spikespace - 1],
    ...
    if (num_blocks != 0) {
        ...
        {
            _run_kernel_synapses_pre_codeobject_3<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
                defaultclock.timestep[0],
                num_threads_per_bundle,
                dev_array_neurongroup_11__spikespace[synapses_pre_eventspace_idx],
                num_spiking_neurons,
                ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_pre_codeobject_3");
}


}

void _debugmsg_synapses_pre_codeobject_3()
{
    using namespace brian;
