#include "code_objects/neurongroup_3_stateupdater_codeobject_4.h"
#include "objects.h"
...
__global__ void
_run_kernel_neurongroup_3_stateupdater_codeobject_4(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_3_A,
	double* _ptr_array_neurongroup_3_Iexc,
	double* _ptr_array_neurongroup_3_Iinh,
    ...
    )
{
...

    {
        ...
        double A = _ptr_array_neurongroup_3_A[_idx];
        const double Iexc = _ptr_array_neurongroup_3_Iexc[_idx];
        const double Iinh = _ptr_array_neurongroup_3_Iinh[_idx];
        double X = _ptr_array_neurongroup_3_X[_idx];
        double Y = _ptr_array_neurongroup_3_Y[_idx];
        const double dt = _ptr_array_defaultclock_dt[0];
        ...
        Y = _Y;
        v = _v;
        _ptr_array_neurongroup_3_A[_idx] = A;
        _ptr_array_neurongroup_3_X[_idx] = X;
        _ptr_array_neurongroup_3_Y[_idx] = Y;
...
void _run_neurongroup_3_stateupdater_codeobject_4()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_3_stateupdater_codeobject_4, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_3_stateupdater_codeobject_4, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_3_stateupdater_codeobject_4)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_3_stateupdater_codeobject_4\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_3_stateupdater_codeobject_4<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_3_A,
			dev_array_neurongroup_3_Iexc,
			dev_array_neurongroup_3_Iinh,
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_3_stateupdater_codeobject_4");
