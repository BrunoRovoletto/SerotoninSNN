#include "code_objects/synapses_4_summed_variable_Iinh_post_codeobject_4.h"
#include "objects.h"
...
__global__ void
_run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_4_Iinh,
	...
	double* _ptr_array_synapses_4_w,
	const int _numw
    )
{
...

    {
        ...
        const int32_t _presynaptic_idx = _ptr_array_synapses_4__synaptic_pre[_idx];
...
int _target_id = _ptr_array_synapses_4__synaptic_post[_idx];
_brian_atomicAdd(&_ptr_array_neurongroup_4_Iinh[_target_id], _synaptic_var);
    }
}



void _run_synapses_4_summed_variable_Iinh_post_codeobject_4()
{
    using namespace brian;
    ...
    const int _numIinh_post = 2;
		...
		double* const dev_array_synapses_4_w = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_4_w[0]);
		const int _numw = dev_dynamic_array_synapses_4_w.size();
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
CUDA_SAFE_CALL(
        cudaMemset(dev_array_neurongroup_4_Iinh + 0,
                   0,
                   _target_size * sizeof(double))
        );

    _run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_4_Iinh,
			...
			_num_synaptic_post,
			dev_array_synapses_4_w,
			_numw
        );

    CUDA_CHECK_ERROR("_run_kernel_synapses_4_summed_variable_Iinh_post_codeobject_4");
