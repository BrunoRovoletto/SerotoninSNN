...
__global__ void
_run_kernel_synapses_3_pre_codeobject_2(
    int _N,
    int bid_offset,
    int timestep,
...
printf("INFO _run_kernel_synapses_3_pre_codeobject_2: Using %d threads per bundle\n", num_threads_per_bundle);
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_3_pre_codeobject_2, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_3_pre_codeobject_2)
                );
...
else if (synapses_3_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_3_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO _run_kernel_synapses_3_pre_codeobject_2\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_3_pre_max_size > 0)
{
    ...
    if (num_blocks != 0) {
        ...
        {
            _run_kernel_synapses_3_pre_codeobject_2<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_3_pre_codeobject_2");
}
