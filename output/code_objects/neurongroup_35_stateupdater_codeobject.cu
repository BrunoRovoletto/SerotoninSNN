#include "code_objects/neurongroup_35_stateupdater_codeobject.h"
#include "objects.h"
...
namespace {
    ...
    float _brian_exp(float value)
    {
        return exp(value);
                        ...
                        #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ > 0))
                        __device__ double* _namespace_timedarray_7_values;
                        #else
                        double* _namespace_timedarray_7_values;
                        #endif
    __host__ __device__
    static inline double _timedarray_7(const double t, const int i)
    {
        ...
        if(timestep < 0)
           timestep = 0;
            ...
            timestep = 40-1;
        return _namespace_timedarray_7_values[timestep*120 + i];
    }
...
__global__ void
_run_kernel_neurongroup_35_stateupdater_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_35_A,
	...
	int32_t* _ptr_array_neurongroup_35_indices,
	const double _value_array_defaultclock_t,
	double* _ptr_array_neurongroup_35_v
    )
{
    ...
    ///// kernel_lines /////
                        ...
                        #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ > 0))
                        _namespace_timedarray_7_values = d_timedarray_7_values;
                        #else
                        _namespace_timedarray_7_values = _timedarray_7_values;
                        #endif
...

    {
        ...
        double A = _ptr_array_neurongroup_35_A[_idx];
        const double Iexc = _ptr_array_neurongroup_35_Iexc[_idx];
        const double Iexc2 = _ptr_array_neurongroup_35_Iexc2[_idx];
        double X = _ptr_array_neurongroup_35_X[_idx];
        double Y = _ptr_array_neurongroup_35_Y[_idx];
        const double dt = _ptr_array_defaultclock_dt[0];
        const int32_t indices = _ptr_array_neurongroup_35_indices[_idx];
        const double t = _ptr_array_defaultclock_t[0];
        double v = _ptr_array_neurongroup_35_v[_idx];
        const double _A = A + (_lio_1 * ((- A) + (c * (_lio_2 + v))));
        const double _X = X + (dt * ((_lio_3 * (- X)) + (_lio_4 * Y)));
        const double _Y = (_lio_5 * (- Y)) + Y;
        const double _v = v + (_lio_6 * ((((((- A) + (_lio_7 * _brian_exp(_lio_8 * (_lio_9 + v)))) + Iexc) + Iexc2) + _timedarray_7(t, indices)) - (gL * (_lio_2 + v))));
        A = _A;
        X = _X;
        Y = _Y;
        v = _v;
        _ptr_array_neurongroup_35_A[_idx] = A;
        _ptr_array_neurongroup_35_X[_idx] = X;
        _ptr_array_neurongroup_35_Y[_idx] = Y;
        _ptr_array_neurongroup_35_v[_idx] = v;
...
void _run_neurongroup_35_stateupdater_codeobject()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_35_stateupdater_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_35_stateupdater_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_35_stateupdater_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_35_stateupdater_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_35_stateupdater_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_35_A,
			...
			dev_array_neurongroup_35_indices,
			_array_defaultclock_t[0],
			dev_array_neurongroup_35_v
        );

    CUDA_CHECK_ERROR("_run_kernel_neurongroup_35_stateupdater_codeobject");
