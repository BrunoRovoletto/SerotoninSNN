...
__global__ void _before_run_kernel_synapses_31_pre_push_spikes(
    int _source_N,
    int _num_blocks,
    int _num_threads,
    double _dt,
    ...
    bool scalar_delay)
{
    ...
    if (scalar_delay)
    {
        if (tid == 0)
        {
            synapses_31_pre.queue->num_blocks = _num_blocks;
            synapses_31_pre.queue->num_delays = num_delays;
        }
    }
    else
    {
        synapses_31_pre.queue->prepare(
            tid,
            _num_threads,
            _num_blocks,
            0,
            ...
            _syn_N,
            num_delays,
            synapses_31_pre_num_synapses_by_pre,
            synapses_31_pre_num_synapses_by_bundle,
            synapses_31_pre_num_unique_delays_by_pre,
            synapses_31_pre_unique_delays,
            synapses_31_pre_global_bundle_id_start_by_pre,
            synapses_31_pre_synapses_offset_by_bundle,
            synapses_31_pre_synapse_ids,
            synapses_31_pre_synapse_ids_by_pre,
            synapses_31_pre_unique_delays_offset_by_pre,
            synapses_31_pre_unique_delay_start_idcs
        );
    }
    synapses_31_pre.no_or_const_delay_mode = scalar_delay;
}
...
void _before_run_synapses_31_pre_push_spikes()
{
    using namespace brian;
    ...
    const int _numN = 1;
		...
		double* const _array_synapses_31_delay = thrust::raw_pointer_cast(&_dynamic_array_synapses_31_delay[0]);
		const int _numdelay = _dynamic_array_synapses_31_delay.size();
		double* const dev_array_synapses_31_delay = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_31_delay[0]);

    ///// pointers_lines /////
        
    int32_t*   _ptr_array_synapses_31_N = _array_synapses_31_N;
    double*   _ptr_array_defaultclock_dt = _array_defaultclock_dt;
    int32_t* __restrict  _ptr_array_neurongroup_15__spikespace = _array_neurongroup_15__spikespace;
    double* __restrict  _ptr_array_synapses_31_delay = _array_synapses_31_delay;


    int64_t syn_N_check = _ptr_array_synapses_31_N[0];
    ...
    int max_delay = (int)(_dynamic_array_synapses_31_delay[0] / dt + 0.5);
    ...
    for(int syn_id = 0; syn_id < syn_N; syn_id++)  // loop through all synapses
    {
        ...
        int32_t pre_neuron_id = _dynamic_array_synapses_31__synaptic_pre[syn_id] - 0;
        int32_t post_neuron_id = _dynamic_array_synapses_31__synaptic_post[syn_id] - 0;

        int delay = (int)(_dynamic_array_synapses_31_delay[syn_id] / dt + 0.5);
        if (delay > max_delay)
            max_delay = delay;
        if (delay < min_delay)
            min_delay = delay;
    ...
    if (scalar_delay)
        synapses_31_pre_delay = max_delay;
    CUDA_CHECK_MEMORY();
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (num_elements > synapses_31_pre_max_size)
            synapses_31_pre_max_size = num_elements;
        ...
        if (!scalar_delay)
        {
            ...
            if (num_unique_elements > synapses_31_pre_max_num_unique_delays)
                synapses_31_pre_max_num_unique_delays = num_unique_elements;
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (bundle_idx == 0){
                    synapses_31_pre_bundle_size_min = num_synapses;
                    synapses_31_pre_bundle_size_max = num_synapses;
                }
                else {
                    if (num_synapses > synapses_31_pre_bundle_size_max){
                        synapses_31_pre_bundle_size_max = num_synapses;
                    }
                    if (num_synapses < synapses_31_pre_bundle_size_min){
                        synapses_31_pre_bundle_size_min = num_synapses;
                    }
                }
    ...
    if (scalar_delay)
    {
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_num_synapses_by_pre,
                h_num_synapses_by_pre, synapses_31_pre_num_synapses_by_pre,
                num_pre_post_blocks, "number of synapses per pre/post block");
        // synapses id
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_d_ptr_synapse_ids_by_pre,
                h_ptr_d_ptr_synapse_ids_by_pre, synapses_31_pre_synapse_ids_by_pre,
                num_pre_post_blocks,
    ...
    else  // not scalar_delay
    {
        ...
        CUDA_SAFE_CALL(
                cudaMemcpyToSymbol(synapses_31_pre_unique_delays,
                                   &d_ptr_unique_delays,
        ...
        synapses_31_pre_bundle_size_mean = mean_bundle_sizes;
        synapses_31_pre_bundle_size_std = getStd(count_bundle_sizes, M2_bundle_sizes);

        // pointer to start of synapse IDs array
        CUDA_SAFE_CALL(
                cudaMemcpyToSymbol(synapses_31_pre_synapse_ids, &d_ptr_synapse_ids,
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_num_synapses_by_bundle,
                ...
                synapses_31_pre_num_synapses_by_bundle, num_bundle_ids,
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_synapses_offset_by_bundle,
                ...
                synapses_31_pre_synapses_offset_by_bundle, num_bundle_ids,
    ...
    std::cout << "INFO: synapse statistics and memory usage for synapses_31_pre:\n"
    ...
    if (scalar_delay)
    {
        int num_eventspaces = dev_array_neurongroup_15__spikespace.size();
        ...
        if (require_new_eventspaces)
        {
            ...
            std::rotate(
                dev_array_neurongroup_15__spikespace.begin(),
                dev_array_neurongroup_15__spikespace.begin() + current_idx_array_neurongroup_15__spikespace,
                dev_array_neurongroup_15__spikespace.end()
            );
            current_idx_array_neurongroup_15__spikespace = 0;
            // add new eventspaces
            for (int i = num_eventspaces; i < num_queues; i++)
            {
                ...
                CUDA_SAFE_CALL(
                    cudaMalloc(
                        (void**)&new_eventspace,
                        sizeof(int32_t) * _num__array_neurongroup_15__spikespace
                    )
                );
                ...
                CUDA_SAFE_CALL(
                    cudaMemcpy(
                        new_eventspace,
                        _array_neurongroup_15__spikespace,  // defined in objects.cu
                        sizeof(int32_t) * _num__array_neurongroup_15__spikespace,
                        cudaMemcpyHostToDevice
                    )
                );
                dev_array_neurongroup_15__spikespace.push_back(new_eventspace);
            }
        }
    }
    ...
    cudaFuncGetAttributes(&funcAttrib, _before_run_kernel_synapses_31_pre_push_spikes);
    if (num_threads > funcAttrib.maxThreadsPerBlock)
    {
        // use the max num_threads before launch failure
        num_threads = funcAttrib.maxThreadsPerBlock;
        printf("WARNING Not enough ressources available to call "
               "_before_run_kernel_synapses_31_pre_push_spikes"
    ...
    {
        printf("INFO _before_run_kernel_synapses_31_pre_push_spikes\n"
               "\t%u blocks\n"
               "\t%u threads\n"
    ...
    _before_run_kernel_synapses_31_pre_push_spikes<<<num_blocks, num_threads>>>(
        source_N,
        num_parallel_blocks,
        num_threads,
        dt,
    ...
    synapses_31_pre_scalar_delay = scalar_delay;

    cudaError_t status = cudaGetLastError();
    if (status != cudaSuccess)
    {
        printf("ERROR initialising synapses_31_pre in %s:%d %s\n",
    ...
    std::cout << "INFO: synapses_31_pre initialisation took " <<  time_passed << "s";
