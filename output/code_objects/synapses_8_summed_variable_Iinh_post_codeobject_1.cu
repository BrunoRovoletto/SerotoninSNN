#include "code_objects/synapses_8_summed_variable_Iinh_post_codeobject_1.h"
#include "objects.h"
...
__global__ void
_run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_5_Iinh,
	...
	double* _ptr_array_synapses_8_w,
	const int _numw
    )
{
...

    {
        ...
        const int32_t _presynaptic_idx = _ptr_array_synapses_8__synaptic_pre[_idx];
        const double X_pre = _ptr_array_neurongroup_2_X[_presynaptic_idx];
        const double w = _ptr_array_synapses_8_w[_idx];
        const double _synaptic_var = w * X_pre;


int _target_id = _ptr_array_synapses_8__synaptic_post[_idx];
_brian_atomicAdd(&_ptr_array_neurongroup_5_Iinh[_target_id], _synaptic_var);
    }
}



void _run_synapses_8_summed_variable_Iinh_post_codeobject_1()
{
    using namespace brian;
    ...
    const int _numIinh_post = 2;
		...
		const int _num_presynaptic_idx = dev_dynamic_array_synapses_8__synaptic_pre.size();
		const int _num_synaptic_post = dev_dynamic_array_synapses_8__synaptic_post.size();
		double* const dev_array_synapses_8_w = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_8_w[0]);
		const int _numw = dev_dynamic_array_synapses_8_w.size();
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
CUDA_SAFE_CALL(
        cudaMemset(dev_array_neurongroup_5_Iinh + 0,
                   0,
                   _target_size * sizeof(double))
        );

    _run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_5_Iinh,
			...
			_num_synaptic_post,
			dev_array_synapses_8_w,
			_numw
        );

    CUDA_CHECK_ERROR("_run_kernel_synapses_8_summed_variable_Iinh_post_codeobject_1");
