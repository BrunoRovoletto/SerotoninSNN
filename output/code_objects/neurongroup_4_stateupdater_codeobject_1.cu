#include "code_objects/neurongroup_4_stateupdater_codeobject_1.h"
#include "objects.h"
...
__global__ void
_run_kernel_neurongroup_4_stateupdater_codeobject_1(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_4_A,
	double* _ptr_array_neurongroup_4_Iexc,
	double* _ptr_array_neurongroup_4_Iinh,
	double* _ptr_array_neurongroup_4_X,
	double* _ptr_array_neurongroup_4_Y,
	const double _value_array_defaultclock_dt,
	double* _ptr_array_neurongroup_4_v
    )
{
...

    {
        ...
        double A = _ptr_array_neurongroup_4_A[_idx];
        const double Iexc = _ptr_array_neurongroup_4_Iexc[_idx];
        const double Iinh = _ptr_array_neurongroup_4_Iinh[_idx];
        double X = _ptr_array_neurongroup_4_X[_idx];
        double Y = _ptr_array_neurongroup_4_Y[_idx];
        const double dt = _ptr_array_defaultclock_dt[0];
        ...
        Y = _Y;
        v = _v;
        _ptr_array_neurongroup_4_A[_idx] = A;
        _ptr_array_neurongroup_4_X[_idx] = X;
        _ptr_array_neurongroup_4_Y[_idx] = Y;
...
void _run_neurongroup_4_stateupdater_codeobject_1()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_4_stateupdater_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_4_stateupdater_codeobject_1, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_4_stateupdater_codeobject_1)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_4_stateupdater_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_4_stateupdater_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_4_A,
			dev_array_neurongroup_4_Iexc,
			dev_array_neurongroup_4_Iinh,
			dev_array_neurongroup_4_X,
			dev_array_neurongroup_4_Y,
			_array_defaultclock_dt[0],
			dev_array_neurongroup_4_v
        );

    CUDA_CHECK_ERROR("_run_kernel_neurongroup_4_stateupdater_codeobject_1");
