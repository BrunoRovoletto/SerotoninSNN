...
__global__ void
_run_kernel_spikemonitor_codeobject(
    int _N,
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_spikemonitor_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_spikemonitor_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_spikemonitor_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_spikemonitor_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
int _monitor_size = dev_dynamic_array_spikemonitor_i.size();
...
if (_N > 0)
{
    ...
    CUDA_CHECK_ERROR("_run_kernel_spikemonitor_codeobject");
}
