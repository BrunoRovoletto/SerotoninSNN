...
__global__ void _advance_kernel_synapses_21_pre_push_spikes()
{
    using namespace brian;
    int tid = threadIdx.x;
    synapses_21_pre.queue->advance(
        tid);
}
...
__global__ void
_run_kernel_synapses_21_pre_push_spikes(
    ...
    int32_t* _eventspace)
{
    ...
    if(synapses_21_pre.spikes_start <= spiking_neuron && spiking_neuron < synapses_21_pre.spikes_stop)
    {
        synapses_21_pre.queue->push_bundles(
            post_neuron_bid,
            tid,
            _num_threads,
            spiking_neuron - synapses_21_pre.spikes_start);
    }
}
...
void _run_synapses_21_pre_push_spikes()
{
    ...
    if (synapses_21_pre_scalar_delay)
    {
        ...
        synapses_21_pre_eventspace_idx = (current_idx_array_neurongroup_13__spikespace - synapses_21_pre_delay + num_eventspaces) % num_eventspaces;
    ...
    else if (synapses_21_pre_max_size > 0)
    {
        ...
        _advance_kernel_synapses_21_pre_push_spikes<<<1, num_parallel_blocks>>>();

        CUDA_CHECK_ERROR("_advance_kernel_synapses_21_pre_push_spikes");
    ...
    num_threads = synapses_21_pre_max_size;
    ...
    // num_blocks depends on num_spiking_neurons, which changes each time step
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_21_pre_push_spikes, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_21_pre_push_spikes)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_21_pre_push_spikes\n"
"\tvariable number of blocks (depends on number of spiking neurons)\n"
        ...
        if (num_spiking_neurons > 0)
        {
            ...
            _run_kernel_synapses_21_pre_push_spikes<<<num_blocks, num_threads, needed_shared_memory>>>(
                    num_parallel_blocks,
                    num_blocks,
                    num_threads,
                    dev_array_neurongroup_13__spikespace[current_idx_array_neurongroup_13__spikespace]);

            CUDA_CHECK_ERROR("_run_kernel_synapses_21_pre_push_spikes");
        }
    }  // end else if (synapses_21_pre_max_size > 0) 
