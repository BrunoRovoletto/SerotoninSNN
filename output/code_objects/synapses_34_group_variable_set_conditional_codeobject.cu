...
__global__ void
_run_kernel_synapses_34_group_variable_set_conditional_codeobject(
    int _N,
    ...
    )
{
    ...
    if (_cond)
    {
        ...
        double w;
        w = w_init;
        _ptr_array_synapses_34_w[_idx] = w;
...
void _run_synapses_34_group_variable_set_conditional_codeobject()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_synapses_34_group_variable_set_conditional_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_34_group_variable_set_conditional_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_34_group_variable_set_conditional_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_34_group_variable_set_conditional_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_synapses_34_group_variable_set_conditional_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_34_group_variable_set_conditional_codeobject");

    _dynamic_array_synapses_34_w = dev_dynamic_array_synapses_34_w;
