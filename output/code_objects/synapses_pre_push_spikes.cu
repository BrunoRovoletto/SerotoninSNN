...
__global__ void _advance_kernel_synapses_pre_push_spikes()
{
...
__global__ void
_run_kernel_synapses_pre_push_spikes(
...
void _run_synapses_pre_push_spikes()
{
    ...
    if (synapses_pre_scalar_delay)
    {
        ...
        synapses_pre_eventspace_idx = (current_idx_array_neurongroup_11__spikespace - synapses_pre_delay + num_eventspaces) % num_eventspaces;
    ...
    else if (synapses_pre_max_size > 0)
    {
        ...
        CUDA_SAFE_CALL(
                cudaMemcpy(&num_spiking_neurons,
                    dev_array_neurongroup_11__spikespace[current_idx_array_neurongroup_11__spikespace] + _num_spikespace - 1,
        ...
        CUDA_CHECK_ERROR("_advance_kernel_synapses_pre_push_spikes");
    ...
    // num_blocks depends on num_spiking_neurons, which changes each time step
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_pre_push_spikes, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_pre_push_spikes)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_pre_push_spikes\n"
"\tvariable number of blocks (depends on number of spiking neurons)\n"
        ...
        if (num_spiking_neurons > 0)
        {
            ...
            _run_kernel_synapses_pre_push_spikes<<<num_blocks, num_threads, needed_shared_memory>>>(
                    num_parallel_blocks,
                    num_blocks,
                    num_threads,
                    dev_array_neurongroup_11__spikespace[current_idx_array_neurongroup_11__spikespace]);

            CUDA_CHECK_ERROR("_run_kernel_synapses_pre_push_spikes");
        }
    }  // end else if (synapses_pre_max_size > 0) 
