#include "code_objects/neurongroup_20_spike_thresholder_codeobject.h"
#include "objects.h"
...
__global__ void
_run_kernel_neurongroup_20_spike_thresholder_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_neurongroup_20__spikespace,
    ...
    )
{
    ...
    const double v = _ptr_array_neurongroup_20_v[_idx];
    const char _cond = v > _lio_1;


    if (_cond)
    {
        int32_t spike_index = atomicAdd(&_ptr_array_neurongroup_20__spikespace[_N], 1);
        _ptr_array_neurongroup_20__spikespace[spike_index] = _idx;
    }
    }
}

__global__ void
_reset_neurongroup_20_spike_thresholder_codeobject(
    int32_t* eventspace
    )
{
    using namespace brian;
...
void _run_neurongroup_20_spike_thresholder_codeobject()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_20_spike_thresholder_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_20_spike_thresholder_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_20_spike_thresholder_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_20_spike_thresholder_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
        ...
        _reset_neurongroup_20_spike_thresholder_codeobject<<<num_blocks, num_threads>>>(
                dev_array_neurongroup_20__spikespace[current_idx_array_neurongroup_20__spikespace]
            );

        CUDA_CHECK_ERROR("_reset_neurongroup_20_spike_thresholder_codeobject");

    _run_kernel_neurongroup_20_spike_thresholder_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_20__spikespace[current_idx_array_neurongroup_20__spikespace],
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_20_spike_thresholder_codeobject");
