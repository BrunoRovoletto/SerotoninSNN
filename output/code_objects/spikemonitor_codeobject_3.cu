...
__global__ void
_run_kernel_spikemonitor_codeobject_3(
    int _N,
...
void _run_spikemonitor_codeobject_3()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_spikemonitor_codeobject_3, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_spikemonitor_codeobject_3, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_spikemonitor_codeobject_3)
                );
        ...
        {
            printf("INFO _run_kernel_spikemonitor_codeobject_3\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (_N > 0)
{
    _run_kernel_spikemonitor_codeobject_3<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            _monitor_size,
            _eventspace,
    ...
    CUDA_CHECK_ERROR("_run_kernel_spikemonitor_codeobject_3");
}
...
void _debugmsg_spikemonitor_codeobject_3()
{
    using namespace brian;
