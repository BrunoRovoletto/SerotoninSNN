...
void _before_run_synapses_2_pre_push_spikes()
{
    ...
    for(int syn_id = 0; syn_id < syn_N; syn_id++)  // loop through all synapses
    {
        ...
        int delay = (int)(_dynamic_array_synapses_2_delay[syn_id] / dt + 0.5);
        if (delay > max_delay)
            max_delay = delay;
        if (delay < min_delay)
            min_delay = delay;
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (!scalar_delay)
        {
            ...
            if (num_unique_elements > synapses_2_pre_max_num_unique_delays)
                synapses_2_pre_max_num_unique_delays = num_unique_elements;
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (bundle_idx == 0){
                    synapses_2_pre_bundle_size_min = num_synapses;
                    synapses_2_pre_bundle_size_max = num_synapses;
                }
                else {
                    if (num_synapses > synapses_2_pre_bundle_size_max){
                        synapses_2_pre_bundle_size_max = num_synapses;
                    }
                    if (num_synapses < synapses_2_pre_bundle_size_min){
                        synapses_2_pre_bundle_size_min = num_synapses;
                    }
                }
    ...
    else  // not scalar_delay
    {
        ...
        CUDA_SAFE_CALL(
                cudaMemcpyToSymbol(synapses_2_pre_unique_delays,
                                   &d_ptr_unique_delays,
        ...
        synapses_2_pre_bundle_size_mean = mean_bundle_sizes;
        synapses_2_pre_bundle_size_std = getStd(count_bundle_sizes, M2_bundle_sizes);

        // pointer to start of synapse IDs array
        CUDA_SAFE_CALL(
                cudaMemcpyToSymbol(synapses_2_pre_synapse_ids, &d_ptr_synapse_ids,
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_num_synapses_by_bundle,
                ...
                synapses_2_pre_num_synapses_by_bundle, num_bundle_ids,
        ...
        COPY_HOST_ARRAY_TO_DEVICE_SYMBOL(d_ptr_synapses_offset_by_bundle,
                ...
                synapses_2_pre_synapses_offset_by_bundle, num_bundle_ids,
    ...
    synapses_2_pre_scalar_delay = scalar_delay;
