...
__global__ void _before_run_kernel_synapses_pre_push_spikes(
    int _source_N,
    int _num_blocks,
    int _num_threads,
    double _dt,
    ...
    bool scalar_delay)
{
    ...
    if (scalar_delay)
    {
        if (tid == 0)
        {
            synapses_pre.queue->num_blocks = _num_blocks;
            synapses_pre.queue->num_delays = num_delays;
        }
    }
    else
    {
    ...
    synapses_pre.no_or_const_delay_mode = scalar_delay;
}
...
void _before_run_synapses_pre_push_spikes()
{
    ...
    const int _numN = 1;
		...
		double* const dev_array_synapses_delay = thrust::raw_pointer_cast(&dev_dynamic_array_synapses_delay[0]);
    ...
    for(int i = 0; i < num_pre_post_blocks; i++)
    {
        ...
        if (!scalar_delay)
        {
            ...
            for (int bundle_idx = 0; bundle_idx < num_unique_elements; bundle_idx++)
            {
                ...
                if (bundle_idx == 0){
                    synapses_pre_bundle_size_min = num_synapses;
                    synapses_pre_bundle_size_max = num_synapses;
                }
                else {
                    if (num_synapses > synapses_pre_bundle_size_max){
                        synapses_pre_bundle_size_max = num_synapses;
                    }
                    if (num_synapses < synapses_pre_bundle_size_min){
                        synapses_pre_bundle_size_min = num_synapses;
                    }
                }
    ...
    else  // not scalar_delay
    {
        ...
        synapses_pre_bundle_size_mean = mean_bundle_sizes;
        synapses_pre_bundle_size_std = getStd(count_bundle_sizes, M2_bundle_sizes);
    ...
    if (scalar_delay)
    {
        int num_eventspaces = dev_array_neurongroup_11__spikespace.size();
        ...
        if (require_new_eventspaces)
        {
            ...
            std::rotate(
                dev_array_neurongroup_11__spikespace.begin(),
                dev_array_neurongroup_11__spikespace.begin() + current_idx_array_neurongroup_11__spikespace,
                dev_array_neurongroup_11__spikespace.end()
            );
            current_idx_array_neurongroup_11__spikespace = 0;
            // add new eventspaces
            for (int i = num_eventspaces; i < num_queues; i++)
            {
                ...
                CUDA_SAFE_CALL(
                    cudaMemcpy(
                        new_eventspace,
                        _array_neurongroup_11__spikespace,  // defined in objects.cu
                        sizeof(int32_t) * _num__array_neurongroup_11__spikespace,
                        cudaMemcpyHostToDevice
                    )
                );
                dev_array_neurongroup_11__spikespace.push_back(new_eventspace);
            }
        }
    }
    ...
    cudaFuncGetAttributes(&funcAttrib, _before_run_kernel_synapses_pre_push_spikes);
    if (num_threads > funcAttrib.maxThreadsPerBlock)
    {
        // use the max num_threads before launch failure
        num_threads = funcAttrib.maxThreadsPerBlock;
        printf("WARNING Not enough ressources available to call "
               "_before_run_kernel_synapses_pre_push_spikes"
    ...
    {
        printf("INFO _before_run_kernel_synapses_pre_push_spikes\n"
               "\t%u blocks\n"
               "\t%u threads\n"
    ...
    _before_run_kernel_synapses_pre_push_spikes<<<num_blocks, num_threads>>>(
        source_N,
        num_parallel_blocks,
        num_threads,
        dt,
