#include "code_objects/synapses_summed_variable_Iexc2_post_codeobject_5.h"
#include "objects.h"
...
__global__ void
_run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5(
    int _N,
    ...
    )
{
...

    {
        ...
        const double X_pre = _ptr_array_neurongroup_11_X[_presynaptic_idx];
...
_brian_atomicAdd(&_ptr_array_neurongroup_11_Iexc2[_target_id], _synaptic_var);
    }
}
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
CUDA_SAFE_CALL(
        cudaMemset(dev_array_neurongroup_11_Iexc2 + 0,
                   0,
                   _target_size * sizeof(double))
        );

    _run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_summed_variable_Iexc2_post_codeobject_5");
