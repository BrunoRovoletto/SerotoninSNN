...
__global__ void
_run_kernel_spikemonitor_3_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
        int _monitor_size,
        int32_t* _eventspace,
        double* _new_ptr_array_spikemonitor_3_t,
        int32_t* _new_ptr_array_spikemonitor_3_i,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_spikemonitor_3_N,
	int32_t* _ptr_array_neurongroup_24_i,
	int32_t* _ptr_array_spikemonitor_3__source_idx,
	const double _value_array_defaultclock_t,
	int32_t* _ptr_array_neurongroup_24__spikespace,
	int32_t* _ptr_array_spikemonitor_3_count,
	int32_t* _ptr_array_spikemonitor_3_i,
	const int _numi,
	double* _ptr_array_spikemonitor_3_t,
	const int _numt
    )
{
...
const int32_t _source_i = _ptr_array_neurongroup_24_i[_idx];
...
// fill the monitors
_new_ptr_array_spikemonitor_3_t[_monitor_idx] = _to_record_t;
_new_ptr_array_spikemonitor_3_i[_monitor_idx] = _to_record_i;
...
void _run_spikemonitor_3_codeobject()
{
    ...
    const int _numN = 1;
		...
		const int _num_spikespace = 3;
		const int _numcount = 2;
		int32_t* const dev_array_spikemonitor_3_i = thrust::raw_pointer_cast(&dev_dynamic_array_spikemonitor_3_i[0]);
		const int _numi = dev_dynamic_array_spikemonitor_3_i.size();
		double* const dev_array_spikemonitor_3_t = thrust::raw_pointer_cast(&dev_dynamic_array_spikemonitor_3_t[0]);
		const int _numt = dev_dynamic_array_spikemonitor_3_t.size();
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_spikemonitor_3_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_spikemonitor_3_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_spikemonitor_3_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_spikemonitor_3_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
int32_t* _eventspace = dev_array_neurongroup_24__spikespace[current_idx_array_neurongroup_24__spikespace];
CUDA_SAFE_CALL(
        cudaMemcpy(
            &_num_events,
...
int _monitor_size = dev_dynamic_array_spikemonitor_3_t.size();

// Increase device vectors based on number of events
THRUST_CHECK_ERROR(
        dev_dynamic_array_spikemonitor_3_t.resize(_monitor_size + _N)
        );
THRUST_CHECK_ERROR(
        dev_dynamic_array_spikemonitor_3_i.resize(_monitor_size + _N)
        );
...
if (_N > 0)
{
    _run_kernel_spikemonitor_3_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            _monitor_size,
            _eventspace,
            thrust::raw_pointer_cast(&dev_dynamic_array_spikemonitor_3_t[0]),
            thrust::raw_pointer_cast(&dev_dynamic_array_spikemonitor_3_i[0]),
            ///// HOST_PARAMETERS /////
            dev_array_spikemonitor_3_N,
			dev_array_neurongroup_24_i,
			dev_array_spikemonitor_3__source_idx,
			_array_defaultclock_t[0],
			dev_array_neurongroup_24__spikespace[current_idx_array_neurongroup_24__spikespace],
			dev_array_spikemonitor_3_count,
			dev_array_spikemonitor_3_i,
			_numi,
			dev_array_spikemonitor_3_t,
			_numt
        );

    CUDA_CHECK_ERROR("_run_kernel_spikemonitor_3_codeobject");
}

// Increase total number of events in monitor
_array_spikemonitor_3_N[0] += _N;
