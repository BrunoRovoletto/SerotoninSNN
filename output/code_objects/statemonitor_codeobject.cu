...
namespace {
    ...
    ///// support_code_lines /////
                        ...
                        __device__ double* _namespace_timedarray_values;
                        #else
                        double* _namespace_timedarray_values;
                        #endif
    __host__ __device__
    static inline double _timedarray(const double t, const int i)
    {
        ...
        if(timestep < 0)
           timestep = 0;
            ...
            timestep = 40-1;
        return _namespace_timedarray_values[timestep*225 + i];
    }
...
__global__ void
_run_kernel_statemonitor_codeobject(
    int _N,
    ...
    )
{
    ...
    ///// kernel_lines /////
                        ...
                        #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ > 0))
                        _namespace_timedarray_values = d_timedarray_values;
                        #else
...

    {
        ...
        const double _source_I = _timedarray(__source_I_neurongroup_t, __source_I_neurongroup_indices);
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_statemonitor_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
    ...
    addresses_monitor__dynamic_array_statemonitor_v.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_v[i][0]));
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_statemonitor_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_statemonitor_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_statemonitor_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (_num__array_statemonitor__indices > 0)
{
    ...
    _run_kernel_statemonitor_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
    ...
    CUDA_CHECK_ERROR("_run_kernel_statemonitor_codeobject");
