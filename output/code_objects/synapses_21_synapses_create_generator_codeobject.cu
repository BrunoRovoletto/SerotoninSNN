...
namespace {
    ...
    #define _rand(vectorisation_idx) (_ptr_array_synapses_21_synapses_create_generator_codeobject_rand[vectorisation_idx])
...
// NOTE: _ptr_array_synapses_21_synapses_create_generator_codeobject_rand is NOT an array
...
CurandBuffer<randomNumber_t> _ptr_array_synapses_21_synapses_create_generator_codeobject_rand(&brian::curand_generator, RAND);
randomNumber_t _host_rand(const int _vectorisation_idx)
{
    return _ptr_array_synapses_21_synapses_create_generator_codeobject_rand[_vectorisation_idx];
}

CurandBuffer<randomNumber_t> _ptr_array_synapses_21_synapses_create_generator_codeobject_randn(&brian::curand_generator, RANDN);
randomNumber_t _host_randn(const int _vectorisation_idx)
{
    return _ptr_array_synapses_21_synapses_create_generator_codeobject_randn[_vectorisation_idx];
}
...
size_t used_device_memory_start = used_device_memory;
    ...
    ///// HOST_CONSTANTS ///////////
    const int _numN = 1;
		int32_t* const _array_synapses_21_N_incoming = thrust::raw_pointer_cast(&_dynamic_array_synapses_21_N_incoming[0]);
		const int _numN_incoming = _dynamic_array_synapses_21_N_incoming.size();
		int32_t* const _array_synapses_21_N_outgoing = thrust::raw_pointer_cast(&_dynamic_array_synapses_21_N_outgoing[0]);
		const int _numN_outgoing = _dynamic_array_synapses_21_N_outgoing.size();
		const int64_t N_post = 2;
		const int64_t N_pre = 2;
		const int64_t _source_offset = 0;
		int32_t* const _array_synapses_21__synaptic_post = thrust::raw_pointer_cast(&_dynamic_array_synapses_21__synaptic_post[0]);
		const int _num_synaptic_post = _dynamic_array_synapses_21__synaptic_post.size();
		int32_t* const _array_synapses_21__synaptic_pre = thrust::raw_pointer_cast(&_dynamic_array_synapses_21__synaptic_pre[0]);
		const int _num_synaptic_pre = _dynamic_array_synapses_21__synaptic_pre.size();
    ...
    const int32_t newsize = _dynamic_array_synapses_21__synaptic_pre.size();
            THRUST_CHECK_ERROR(
                    dev_dynamic_array_synapses_21__synaptic_post.resize(newsize)
                    );
            _dynamic_array_synapses_21__synaptic_post.resize(newsize);
            THRUST_CHECK_ERROR(
                    dev_dynamic_array_synapses_21__synaptic_pre.resize(newsize)
                    );
            _dynamic_array_synapses_21__synaptic_pre.resize(newsize);
            THRUST_CHECK_ERROR(
                    dev_dynamic_array_synapses_21_delay.resize(newsize)
                    );
            _dynamic_array_synapses_21_delay.resize(newsize);
            THRUST_CHECK_ERROR(
                    dev_dynamic_array_synapses_21_w.resize(newsize)
                    );
            _dynamic_array_synapses_21_w.resize(newsize);
    ...
    for (int _i=0; _i<newsize; _i++)
    {
        ...
        const std::pair<int32_t, int32_t> source_target = std::pair<int32_t, int32_t>(_dynamic_array_synapses_21__synaptic_pre[_i], _dynamic_array_synapses_21__synaptic_post[_i]);
        source_target_count[source_target]++;
        //printf("source target count = %i\n", source_target_count[source_target]);
        if (source_target_count[source_target] > 1)
        {
            synapses_21_multiple_pre_post = true;
            break;
        }
    }

    // copy changed host data to device
    dev_dynamic_array_synapses_21_N_incoming = _dynamic_array_synapses_21_N_incoming;
    dev_dynamic_array_synapses_21_N_outgoing = _dynamic_array_synapses_21_N_outgoing;
    dev_dynamic_array_synapses_21__synaptic_pre = _dynamic_array_synapses_21__synaptic_pre;
    dev_dynamic_array_synapses_21__synaptic_post = _dynamic_array_synapses_21__synaptic_post;
    CUDA_SAFE_CALL(
            cudaMemcpy(dev_array_synapses_21_N,
                _array_synapses_21_N,
                sizeof(int32_t),
...
_ptr_array_synapses_21_synapses_create_generator_codeobject_rand.free_memory();
_ptr_array_synapses_21_synapses_create_generator_codeobject_randn.free_memory();
...
std::cout << "INFO: synapses_21 creation took " <<  time_passed << "s";
if (tot_memory_MB > 0)
