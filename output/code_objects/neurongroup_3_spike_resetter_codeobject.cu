...
__global__ void
_run_kernel_neurongroup_3_spike_resetter_codeobject(
    int _N,
...
void _run_neurongroup_3_spike_resetter_codeobject()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_3_spike_resetter_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_3_spike_resetter_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_3_spike_resetter_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_3_spike_resetter_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_3_spike_resetter_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_3_spike_resetter_codeobject");
