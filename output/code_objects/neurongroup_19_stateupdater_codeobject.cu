#include "code_objects/neurongroup_19_stateupdater_codeobject.h"
#include "objects.h"
    ...
    )
{
...

    {
        ...
        double A = _ptr_array_neurongroup_19_A[_idx];
        const double Iexc = _ptr_array_neurongroup_19_Iexc[_idx];
        const double Iinh = _ptr_array_neurongroup_19_Iinh[_idx];
        double X = _ptr_array_neurongroup_19_X[_idx];
        double Y = _ptr_array_neurongroup_19_Y[_idx];
        const double dt = _ptr_array_defaultclock_dt[0];
        double v = _ptr_array_neurongroup_19_v[_idx];
        ...
        Y = _Y;
        v = _v;
        _ptr_array_neurongroup_19_A[_idx] = A;
        _ptr_array_neurongroup_19_X[_idx] = X;
        _ptr_array_neurongroup_19_Y[_idx] = Y;
        _ptr_array_neurongroup_19_v[_idx] = v;
...
void _run_neurongroup_19_stateupdater_codeobject()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_19_stateupdater_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_19_stateupdater_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_19_stateupdater_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_19_stateupdater_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_19_stateupdater_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_19_A,
			dev_array_neurongroup_19_Iexc,
			dev_array_neurongroup_19_Iinh,
			dev_array_neurongroup_19_X,
			dev_array_neurongroup_19_Y,
			_array_defaultclock_dt[0],
			dev_array_neurongroup_19_v
        );

    CUDA_CHECK_ERROR("_run_kernel_neurongroup_19_stateupdater_codeobject");
