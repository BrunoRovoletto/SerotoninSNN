#include "code_objects/neurongroup_24_stateupdater_codeobject.h"
#include "objects.h"
...
__global__ void
_run_kernel_neurongroup_24_stateupdater_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    double* _ptr_array_neurongroup_24_A,
	double* _ptr_array_neurongroup_24_X,
	double* _ptr_array_neurongroup_24_Y,
    ...
    )
{
...

    {
        ...
        double A = _ptr_array_neurongroup_24_A[_idx];
        double X = _ptr_array_neurongroup_24_X[_idx];
        double Y = _ptr_array_neurongroup_24_Y[_idx];
        const double dt = _ptr_array_defaultclock_dt[0];
        const int32_t indices = _ptr_array_neurongroup_24_indices[_idx];
        const double t = _ptr_array_defaultclock_t[0];
        double v = _ptr_array_neurongroup_24_v[_idx];
        const double _A = A + (_lio_1 * ((- A) + (c * (_lio_2 + v))));
        const double _X = X + (dt * ((_lio_3 * (- X)) + (_lio_4 * Y)));
        const double _Y = (_lio_5 * (- Y)) + Y;
        const double _v = v + (_lio_6 * ((((- A) + (_lio_7 * _brian_exp(_lio_8 * (_lio_9 + v)))) + _timedarray_1(t, indices)) - (gL * (_lio_2 + v))));
        A = _A;
        X = _X;
        Y = _Y;
        v = _v;
        _ptr_array_neurongroup_24_A[_idx] = A;
        _ptr_array_neurongroup_24_X[_idx] = X;
        _ptr_array_neurongroup_24_Y[_idx] = Y;
        _ptr_array_neurongroup_24_v[_idx] = v;
...
void _run_neurongroup_24_stateupdater_codeobject()
{
    using namespace brian;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_24_stateupdater_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_24_stateupdater_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_24_stateupdater_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_24_stateupdater_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_24_stateupdater_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
            dev_array_neurongroup_24_A,
			dev_array_neurongroup_24_X,
			dev_array_neurongroup_24_Y,
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_24_stateupdater_codeobject");
