#include "code_objects/synapses_39_group_variable_set_conditional_codeobject.h"
#include "objects.h"
...
namespace {
    ...
    #define _rand(vectorisation_idx) (_ptr_array_synapses_39_group_variable_set_conditional_codeobject_rand[vectorisation_idx])
...
__global__ void
_run_kernel_synapses_39_group_variable_set_conditional_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_synapses_39_N,
	double* _ptr_array_synapses_39_group_variable_set_conditional_codeobject_rand,
    ...
    )
{
    ...
    if (_cond)
    {
        ...
        double w;
        w = w_init * _rand(_vectorisation_idx + 0 * _N);
        _ptr_array_synapses_39_w[_idx] = w;
...
void _run_synapses_39_group_variable_set_conditional_codeobject()
{
    using namespace brian;
    ...
    ///// ADDITIONAL_HOST_CODE /////
                            ...
                            double* dev_array_rand;
                            CUDA_SAFE_CALL(
                                cudaMalloc(
                                    (void**)&dev_array_rand,
                                    sizeof(double)*_rand_N*1
                                )
                            );
                            CUDA_SAFE_CALL(
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_synapses_39_group_variable_set_conditional_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_39_group_variable_set_conditional_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_39_group_variable_set_conditional_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_synapses_39_group_variable_set_conditional_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_synapses_39_group_variable_set_conditional_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_39_group_variable_set_conditional_codeobject");

    _dynamic_array_synapses_39_w = dev_dynamic_array_synapses_39_w;
