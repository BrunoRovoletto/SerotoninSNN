    ...
    static inline double _timedarray_1(const double t, const int i)
    {
        ...
        if(timestep < 0)
           timestep = 0;
            ...
            timestep = 40-1;
        return _namespace_timedarray_1_values[timestep*225 + i];
    }
...
__global__ void
_run_kernel_statemonitor_2_codeobject(
    int _N,
	...
	int32_t* _ptr_array_neurongroup_24_indices,
	const double _value_array_defaultclock_t,
	int32_t* _ptr_array_statemonitor_2__indices,
	double* _ptr_array_neurongroup_24_v,
    ...
    )
{
    ...
    ///// KERNEL_CONSTANTS /////
    const int _numN = 1;
	const int _num__source_I_neurongroup_24_indices = 2;
    ...
    _idx = _ptr_array_statemonitor_2__indices[_vectorisation_idx];
...

    {
        ...
        const int32_t __source_I_neurongroup_24_indices = _ptr_array_neurongroup_24_indices[_idx];
        const double __source_I_neurongroup_24_t = _ptr_array_defaultclock_t[0];
        const double _source_v = _ptr_array_neurongroup_24_v[_idx];
        const double _to_record_v = _source_v;
        const double _source_I = _timedarray_1(__source_I_neurongroup_24_t, __source_I_neurongroup_24_indices);
...
void _run_statemonitor_2_codeobject()
{
    ...
    const int _numN = 1;
		...
		const int _num_indices = 1;
		const int _num_source_v = 2;
		double* const dev_array_statemonitor_2_t = thrust::raw_pointer_cast(&dev_dynamic_array_statemonitor_2_t[0]);
...
const int _numt_host = _dynamic_array_statemonitor_2_t.size();

// We push t only on host and don't make a device->host copy in write_arrays()
_dynamic_array_statemonitor_2_t.push_back(defaultclock.t[0]);

// Update size variables for Python side indexing to work
_array_statemonitor_2_N[0] += 1;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_statemonitor_2_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
...
addresses_monitor__dynamic_array_statemonitor_2_I.clear();
addresses_monitor__dynamic_array_statemonitor_2_v.clear();
for(int i = 0; i < _num__array_statemonitor_2__indices; i++)
{
    _dynamic_array_statemonitor_2_I[i].resize(_numt_host + num_iterations - current_iteration);
    addresses_monitor__dynamic_array_statemonitor_2_I.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_2_I[i][0]));
    _dynamic_array_statemonitor_2_v[i].resize(_numt_host + num_iterations - current_iteration);
    addresses_monitor__dynamic_array_statemonitor_2_v.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_2_v[i][0]));
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_statemonitor_2_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_statemonitor_2_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_statemonitor_2_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if(current_iteration >= num_iterations)
{
    for(int i = 0; i < _num__array_statemonitor_2__indices; i++)
    {
        _dynamic_array_statemonitor_2_I[i].resize(_numt_host + 1);
        addresses_monitor__dynamic_array_statemonitor_2_I[i] = thrust::raw_pointer_cast(&_dynamic_array_statemonitor_2_I[i][0]);
        _dynamic_array_statemonitor_2_v[i].resize(_numt_host + 1);
...
if (_num__array_statemonitor_2__indices > 0)
{
    ...
    _run_kernel_statemonitor_2_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
    current_iteration - start_offset,
    thrust::raw_pointer_cast(&addresses_monitor__dynamic_array_statemonitor_2_I[0]),
    thrust::raw_pointer_cast(&addresses_monitor__dynamic_array_statemonitor_2_v[0]),
            ///// HOST_PARAMETERS /////
            dev_array_statemonitor_2_N,
			dev_array_neurongroup_24_indices,
			_array_defaultclock_t[0],
			dev_array_statemonitor_2__indices,
			dev_array_neurongroup_24_v,
			dev_array_statemonitor_2_t,
			_numt
        );

    CUDA_CHECK_ERROR("_run_kernel_statemonitor_2_codeobject");
