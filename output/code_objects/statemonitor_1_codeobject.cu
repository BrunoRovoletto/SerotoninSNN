...
namespace {
    ...
    ///// support_code_lines /////
                        ...
                        __device__ double* _namespace_timedarray_21_values;
                        #else
                        double* _namespace_timedarray_21_values;
                        #endif
    __host__ __device__
    static inline double _timedarray_21(const double t, const int i)
    {
        ...
        if(timestep < 0)
           timestep = 0;
            ...
            timestep = 40-1;
        return _namespace_timedarray_21_values[timestep*225 + i];
    }
...
__global__ void
_run_kernel_statemonitor_1_codeobject(
    int _N,
    ...
    double** monitor_v,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_statemonitor_1_N,
	int32_t* _ptr_array_neurongroup_12_indices,
	const double _value_array_defaultclock_t,
	int32_t* _ptr_array_statemonitor_1__indices,
	double* _ptr_array_neurongroup_12_v,
	double* _ptr_array_statemonitor_1_t,
	const int _numt
    )
{
    ...
    ///// KERNEL_CONSTANTS /////
    const int _numN = 1;
	const int _num__source_I_neurongroup_12_indices = 2;
    ...
    ///// kernel_lines /////
                        ...
                        #if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ > 0))
                        _namespace_timedarray_21_values = d_timedarray_21_values;
                        #else
                        _namespace_timedarray_21_values = _timedarray_21_values;
                        #endif
    ...
    _idx = _ptr_array_statemonitor_1__indices[_vectorisation_idx];
...

    {
        ...
        const int32_t __source_I_neurongroup_12_indices = _ptr_array_neurongroup_12_indices[_idx];
        const double __source_I_neurongroup_12_t = _ptr_array_defaultclock_t[0];
        const double _source_v = _ptr_array_neurongroup_12_v[_idx];
        const double _to_record_v = _source_v;
        const double _source_I = _timedarray_21(__source_I_neurongroup_12_t, __source_I_neurongroup_12_indices);
...
void _run_statemonitor_1_codeobject()
{
    ...
    const int _numN = 1;
		...
		const int _num_indices = 1;
		const int _num_source_v = 2;
		double* const dev_array_statemonitor_1_t = thrust::raw_pointer_cast(&dev_dynamic_array_statemonitor_1_t[0]);
...
const int _numt_host = _dynamic_array_statemonitor_1_t.size();

// We push t only on host and don't make a device->host copy in write_arrays()
_dynamic_array_statemonitor_1_t.push_back(defaultclock.t[0]);

// Update size variables for Python side indexing to work
_array_statemonitor_1_N[0] += 1;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_statemonitor_1_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
...
addresses_monitor__dynamic_array_statemonitor_1_I.clear();
addresses_monitor__dynamic_array_statemonitor_1_v.clear();
for(int i = 0; i < _num__array_statemonitor_1__indices; i++)
{
    _dynamic_array_statemonitor_1_I[i].resize(_numt_host + num_iterations - current_iteration);
    addresses_monitor__dynamic_array_statemonitor_1_I.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_1_I[i][0]));
    _dynamic_array_statemonitor_1_v[i].resize(_numt_host + num_iterations - current_iteration);
    addresses_monitor__dynamic_array_statemonitor_1_v.push_back(thrust::raw_pointer_cast(&_dynamic_array_statemonitor_1_v[i][0]));
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_statemonitor_1_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_statemonitor_1_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_statemonitor_1_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if(current_iteration >= num_iterations)
{
    for(int i = 0; i < _num__array_statemonitor_1__indices; i++)
    {
        _dynamic_array_statemonitor_1_I[i].resize(_numt_host + 1);
        addresses_monitor__dynamic_array_statemonitor_1_I[i] = thrust::raw_pointer_cast(&_dynamic_array_statemonitor_1_I[i][0]);
        _dynamic_array_statemonitor_1_v[i].resize(_numt_host + 1);
        addresses_monitor__dynamic_array_statemonitor_1_v[i] = thrust::raw_pointer_cast(&_dynamic_array_statemonitor_1_v[i][0]);
    }
}

// TODO we get invalid launch configuration if this is 0, which happens e.g. for StateMonitor(..., variables=[])
if (_num__array_statemonitor_1__indices > 0)
{

    _run_kernel_statemonitor_1_codeobject<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
    current_iteration - start_offset,
    thrust::raw_pointer_cast(&addresses_monitor__dynamic_array_statemonitor_1_I[0]),
    thrust::raw_pointer_cast(&addresses_monitor__dynamic_array_statemonitor_1_v[0]),
            ///// HOST_PARAMETERS /////
            dev_array_statemonitor_1_N,
			dev_array_neurongroup_12_indices,
			_array_defaultclock_t[0],
			dev_array_statemonitor_1__indices,
			dev_array_neurongroup_12_v,
			dev_array_statemonitor_1_t,
			_numt
        );

    CUDA_CHECK_ERROR("_run_kernel_statemonitor_1_codeobject");
