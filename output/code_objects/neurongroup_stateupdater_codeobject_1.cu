...
__global__ void
_run_kernel_neurongroup_stateupdater_codeobject_1(
    int _N,
    ...
    )
{
...

    {
        ...
        const double _v = v + (_lio_6 * ((((- A) + (_lio_7 * _brian_exp(_lio_8 * (_lio_9 + v)))) + _timedarray_3(t, indices)) - (gL * (_lio_2 + v))));
        A = _A;
        X = _X;
        Y = _Y;
        v = _v;
...
void _run_neurongroup_stateupdater_codeobject_1()
{
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_neurongroup_stateupdater_codeobject_1, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_neurongroup_stateupdater_codeobject_1, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_neurongroup_stateupdater_codeobject_1)
                );
        ...
        {
            printf("INFO _run_kernel_neurongroup_stateupdater_codeobject_1\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
    ...
    _run_kernel_neurongroup_stateupdater_codeobject_1<<<num_blocks, num_threads>>>(
            _N,
            num_threads,
            ///// HOST_PARAMETERS /////
    ...
    CUDA_CHECK_ERROR("_run_kernel_neurongroup_stateupdater_codeobject_1");
