...
__global__ void
_run_kernel_spikemonitor_2_codeobject(
    int _N,
    int THREADS_PER_BLOCK,
        int _monitor_size,
        int32_t* _eventspace,
        double* _new_ptr_array_spikemonitor_2_t,
        int32_t* _new_ptr_array_spikemonitor_2_i,
    ///// KERNEL_PARAMETERS /////
    int32_t* _ptr_array_spikemonitor_2_N,
	int32_t* _ptr_array_neurongroup_23_i,
...
const int32_t _source_i = _ptr_array_neurongroup_23_i[_idx];
...
// fill the monitors
_new_ptr_array_spikemonitor_2_t[_monitor_idx] = _to_record_t;
_new_ptr_array_spikemonitor_2_i[_monitor_idx] = _to_record_i;
    ...
    if (first_run)
    {
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxPotentialBlockSize(&min_num_threads, &num_threads,
                    _run_kernel_spikemonitor_2_codeobject, 0, 0)  // last args: dynamicSMemSize, blockSizeLimit
                );
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_spikemonitor_2_codeobject, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_spikemonitor_2_codeobject)
                );
        ...
        {
            printf("INFO _run_kernel_spikemonitor_2_codeobject\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
int32_t* _eventspace = dev_array_neurongroup_23__spikespace[current_idx_array_neurongroup_23__spikespace];
CUDA_SAFE_CALL(
        cudaMemcpy(
            &_num_events,
...
int _monitor_size = dev_dynamic_array_spikemonitor_2_t.size();

// Increase device vectors based on number of events
THRUST_CHECK_ERROR(
        dev_dynamic_array_spikemonitor_2_t.resize(_monitor_size + _N)
        );
THRUST_CHECK_ERROR(
        dev_dynamic_array_spikemonitor_2_i.resize(_monitor_size + _N)
        );
...
if (_N > 0)
{
    ...
    CUDA_CHECK_ERROR("_run_kernel_spikemonitor_2_codeobject");
}

// Increase total number of events in monitor
_array_spikemonitor_2_N[0] += _N;
