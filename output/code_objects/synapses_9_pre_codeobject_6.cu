#include "code_objects/synapses_9_pre_codeobject_6.h"
#include "objects.h"
...
__global__ void
_run_kernel_synapses_9_pre_codeobject_6(
    int _N,
    int bid_offset,
    int timestep,
...
void _run_synapses_9_pre_codeobject_6()
{
    using namespace brian;
...
printf("INFO _run_kernel_synapses_9_pre_codeobject_6: Using %d threads per bundle\n", num_threads_per_bundle);
num_loops = 1;
        ...
        CUDA_SAFE_CALL(
                cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks,
                    _run_kernel_synapses_9_pre_codeobject_6, num_threads, needed_shared_memory)
                );
        ...
        CUDA_SAFE_CALL(
                cudaFuncGetAttributes(&funcAttrib, _run_kernel_synapses_9_pre_codeobject_6)
                );
...
else if (synapses_9_pre_max_size <= 0)
{
    printf("INFO there are no synapses in the synapses_9_pre pathway. Skipping synapses_push and synapses kernels.\n");
        ...
        {
            printf("INFO _run_kernel_synapses_9_pre_codeobject_6\n"
                   "\t%u blocks\n"
                   "\t%u threads\n"
...
if (synapses_9_pre_max_size > 0)
{
    ...
    if (num_blocks != 0) {
        ...
        {
            _run_kernel_synapses_9_pre_codeobject_6<<<num_blocks, num_threads>>>(
                _N,
                bid_offset,
    ...
    CUDA_CHECK_ERROR("_run_kernel_synapses_9_pre_codeobject_6");
}


}

void _debugmsg_synapses_9_pre_codeobject_6()
{
    using namespace brian;
